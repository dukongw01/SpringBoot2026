...
public class BoardController {
	...
	public String boardWritePro(BoardDTO bdto, HttpSession session) {
		...
		//session.setAttritube("loginmember")저장한 데이터를 꺼내와햐 한다.
		//세션에서 값 꺼내오는 메소드 session.getAttribute("loginmember")
		//Session 자바의 Object 최상위 객체로 다운캐스팅 해야한다.
		//로그인 id => admin9867의 정보 한 행이 모두 MemberDTO타입으로
		//loginedMember에 저장된다.
		Object loginedMember = (MemberDTO)session.getAttribute("loginmember");
...
//	}
	...
	public String boardList(Model model,
			...
			//페이지 번호 = > 1부터 시작, 초기값 1로 지정
			@RequestParam(value="page", defaultValue = "1") int page,
			//페이지 사이즈 => 한 화면에 보여지는 게시글의 개수를 5로 초기화
...
//			listboard = boardservice.allBoard();
		...
		model.addAttribute("ph", ph); //PageHendler 클래스를 인스턴스한 참조 변수
		String nextPage = "board/boardList";
		return nextPage;
	}
